...
func (k Keeper) AllocateTokens(ctx sdk.Context, totalPreviousPower int64,
	...
	if !feesToCommunity.IsZero() {
		...
		k.SetFeePool(ctx, feePool)
		feeToCommunity := common.ConvertDecToFloat64(feesToCommunity.AmountOf(common.NativeToken))
		k.feeInfo.FeeToCommunityPool += feeToCommunity
		k.metric.FeeToCommunityPool.Add(feeToCommunity)
		logger.Debug("Send fees to community pool", "community_pool", feesToCommunity)
	}
	logger.Error("Total fee to community pool", "community_pool", k.feeInfo.FeeToCommunityPool)
	logger.Error("Total fee to controlled validators", "controlled_validators", k.feeInfo.FeeToControlledVals)
	logger.Error("Total fee to other validators", "other_validators", k.feeInfo.FeeToOtherVals)
	logger.Error("Total fee", "total_fee", k.feeInfo.TotalFee)
}
